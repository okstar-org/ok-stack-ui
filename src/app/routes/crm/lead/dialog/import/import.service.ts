...
export class ImportService {
  ...
  importBegin(params = {}) {
    return this.http
      .get<Payload>(CRM_API.lead.importBegin, { params })
      .pipe(map((r: any) => r.payload));
  }

  importAdd(params = {}): Observable<Payload> {
    this.logger.debug('save', params);
    return this.http.post<Payload>(CRM_API.lead.importAdd, params).pipe(map((r: any) => r.payload));
  }

  importCommit(params = {}) {
    return this.http
      .get<Payload>(CRM_API.lead.importCommit, { params })
      .pipe(map((r: any) => r.payload));
  }
}
